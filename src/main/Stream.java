...
import main.Event.EventId;
import main.Event.EventInputEntry;
import utils.Helper;
...
public class Stream {
			...
			if (toleranceMinutes >= 0) {
				...
				for (int i = 0; i < events.size(); i++) {
					if (toleranceMinutes > 0) {
						...
						addEventToXmlFile(eventsXmlFile, event.getId(), delay, event.getEventStart());
	...
	private void addEventToXmlFile(Element events, EventId id, int eventDelay, long eventStart) {
	...
	private void writeEventXmlFile() {
		try {
			...
			StreamResult result = new StreamResult(new File(GlobalParameters.OUTPUT_DIR + "/events.xml"));
	...
	private void initIdfWindow(Map<String, Integer> amountTermsFraction, int tweetAmountInMinuteWindow, int idfWindowCount) {
		...
		for (Event eventInput : events) {
			if (eventInput.getStartDelay() <= allIdfWindows) {
				...
				for (Entry<String, List<EventInputEntry>> entry : eventInput.getTermEventInputEntries().entrySet()) {
					List<EventInputEntry> eventEntries = entry.getValue();
